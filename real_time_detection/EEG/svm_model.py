...
class SvmModel():
    '''
        ...
        arousal as continuous value and treat this preblem as a regression
        problem.
...
#            self.test_arousal.append(int(label['arousal'] > 5))
    ...
    def evalute(self):
        ...
        import math
        self.train_X = np.array(self.train_X)
        (trian_predict_valence, trian_predict_arousal) =\
        self.predict(self.train_X)
        
        print ('Train valence RMSE:%f' % 
               math.sqrt(mean_squared_error(trian_predict_valence, 
                                            self.train_valence)))
        print ('Train arousal RMSE:%f' %
               math.sqrt(mean_squared_error(trian_predict_arousal, 
                                            self.train_arousal)))
