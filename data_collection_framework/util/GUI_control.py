...
def close_browser():
    '''
        ...
        TODO: make it work even in Linux, it only works in windows because the
      commond belongs to windows. 
    '''
    
    #When the operate system is windows
    bowers_process_names = ['Chrome.exe','Firefox.exe','iexplore.exe','360SE.exe', 
                            'sogouexplorer.exe', 'opera.exe',
                            'Safari.exe','Maxthon.exe', 'Netscape.exe']
    for name in bowers_process_names:
        process_exist, pid = find_pid_from_name(name)
        if process_exist:
            commond =  'taskkill /PID ' + pid +' /F'
            os.popen(commond).read()


def find_pid_from_output(output):
    '''
        This method is used for finding the pid from commond's one-line output.
        We only kill the process whose status is listening.
        
        Arguments:
        
            output: a string from commond
...
def kill_process_by_port(port):
    '''
        ...
        so far, since its API belongs to Windows. Specially, 'the kill_commond'
    ...
    find_port_id_commond = 'netstat -ano | findstr :5000'
    outputs = os.popen(find_port_id_commond).read().split('\n')
...
#    TCP    127.0.0.1:65262        127.0.0.1:5000         ESTABLISHED     8032    
    ...
    for output in outputs:
        ...
        if pid != 0:
            kill_commond = 'taskkill /PID ' + str(pid) +' /F'
            os.popen(kill_commond)