...
@author: Yongrui Huang
  ...
  This is an example for processing adaboost fusion algorithm in DEAP dataset using the function in ../algorithm/adaboost_tool
...
sys.path.append('..')
import numpy as np
from algorithm_implement import adaboost_tool
import configuration

if __name__ == '__main__':
    ROOT_PATH = configuration.DATASET_PATH + 'DEAP/'
    
    for subject_id in range(1, 23):
        subject_path = ROOT_PATH + str(subject_id)+'/'
        
        adaboost_model = adaboost_tool.AdaboostModel(preprocessed=True)
